...
class DynamoCandidaturasRepository:
    """Repository implementation backed by DynamoDB for Candidatura entities."""

    def __init__(self, service: DynamoDBService, *, partition_key: str = "company_id") -> None:
        self._service = service
        self._partition_key = partition_key

    def _build_key(self, identifier: str) -> Dict[str, Any]:
        if not identifier:
            raise InvalidIdentifierError("Candidatura identifier cannot be empty.")
    ...
    async def create(self, data: Dict[str, Any]) -> str:
        ...
        if not company_id:
            raise InvalidIdentifierError("Company identifier is required to create a candidatura.")

        payload = {self._partition_key: company_id, **data}
